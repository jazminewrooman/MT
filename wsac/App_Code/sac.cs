...
public class sac : Isac
{
	...
	public int AltaUsr(MasTicket.Usuario u)
	{
		int res = 0;
		Random r = new Random();
		...
		int nip = 0;
		int b = 0;
		if (Regex.IsMatch(u.email, emailRegex, RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(250)) && int.TryParse(u.nip, out nip) && Regex.IsMatch(u.name, nameRegex, RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds(250)))
		{
			string cs = ConfigurationManager.ConnectionStrings["ac"].ConnectionString;
			DataSet ds = new DataSet();
			using (var conn = new MySqlConnection(cs))
			{
				...
				using (var cmd = new MySqlCommand(command, conn))
				{
					...
					cmd.Parameters.Add("@activo", MySqlDbType.Int32, 1).Value = 0;
					cmd.Parameters.Add("@verificacion", MySqlDbType.VarChar, 10).Value = u.verificacion;
					conn.Open();
					if (cmd.ExecuteNonQuery() > 0)
					{
						res = (int)cmd.LastInsertedId;
    ...
    public string CodigoReferidoUsr(int idusr)
    {
        string stm = "";
        string codigo = "";
        ...
        else
        {
            ...
            ExecuteSQL("insert into codigosreferidos values(" + idusr.ToString() + ", '" + codigo + "', now(), 1)");
        }
        return (codigo);
    }
    ...
    */
	...
	public string GetAClientes()
	{
		...
		using (var conn = new MySqlConnection(cs))
		{
			string command = "Select count(u.idusuario) as altas ,week(u.fechaalta)  as semana , year(u.fechaalta) as anio ,(select sum(p.monto) from recarga r inner join catpaquete p on p.idpaquete = r.idpaquete where week(r.fecha) = semana) ventas from usuario u group by anio,semana limit 100;";
			using (var cmd = new MySqlCommand(command, conn))
			{
				MySqlDataAdapter adapt = new MySqlDataAdapter(cmd);
				conn.Open();
...
where r.fecha between str_to_date('" + fini + @"', '%Y%m%d') and str_to_date('" + ffin + @"', '%Y%m%d')");
	...
	private void EnviaMail(string email, char tipo, string liga, Usuario u = null, byte[] reporte = null, Recarga r = null, catPaquete cp = null)
	{
